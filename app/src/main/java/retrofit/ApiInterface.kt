...
interface ApiInterface {
    ...
    ): Response<ResponseWrapper<Session>>
    ...
    @GET("inquiry")
    suspend fun inquiryGetSupportedPlans(
        @Query("insurance_cover") insuranceCover: Int,
        @Query("is_entry") isEntry: Int,
        @Query("birthdays") birthdays: String,
        @Query("start_at") startAt: String,
        @Query("end_at") endAt: String
    ): Response<ResponseWrapper<List<SupportedPlan>>>
    ...
    suspend fun submit(
        ...
        @Field("deductible_ids") deductibleIds: String,
